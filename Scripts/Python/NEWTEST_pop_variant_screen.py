...
regions=pd.read_table(input_file, sep="\t", names=["cro", "start", "end","gene"], dtype='object', engine='c')
...
if mode == "PROMPT":
    ...
    def merge_and_filter(fill, s_vep):
        left=pd.read_table(fill, sep="\t", names=["chr", "ps", "id", "ref", "alt", "ac", "an", "af", "maf", "ac_het", "ac_hom", "ac_hemi"],engine='c')
        #right=pd.read_table(s_vep, sep="\t", names=["id", "sym", "feat", "cons", "gnomAF", "CDSps", "exon", "intr", "prot_ps", "AmAc", "sift", "polyphen", "cadd", "carr"],engine='c')
        merge=pd.DataFrame()
        for chunk in pd.read_table(s_vep, sep="\t", names=["id", "sym", "feat", "cons", "gnomAF", "CDSps", "exon", "intr", "prot_ps", "AmAc", "sift", "polyphen", "cadd", "carr"],engine='c', chunksize=5000):
            merge=pd.concat([merge,left.merge(chunk, on='id', how="inner")])
    ...
    #dup=merge.drop_duplicates()
        ...
        if dup["cons"].isin(effect_var).any():
            with open("Variant_search_" + pop_input + ".txt", "a+") as final:
                ...
                data_ok=drop.to_string(header=head, na_rep="NA", index=False)
                final.write(data_ok)
                final.close()
        else:
