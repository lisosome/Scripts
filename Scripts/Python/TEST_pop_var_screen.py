...
regions=pd.read_table(input_file, sep="\t", names=["cro", "start", "end","gene"], dtype='object')

def bcf_search(vcf):
    if pop_input != "SR":
        split_fields="%ID\\t%SYMBOL\\t%Feature\\t%Consequence\\t%gnomAD_AF\\t%CDS_position\\t%EXON\\t%INTRON\\t%Protein_position\\t%Amino_acids\\t%SIFT\\t%PolyPhen\\t%CADD_RAW[\\t%SAMPLE=%GT]\\n"
...
def merge_and_filter(fill, s_vep):
    left=pd.read_table(fill, sep="\t", names=["chr", "ps", "id", "ref", "alt", "ac", "an", "af", "maf", "ac_het", "ac_hom", "ac_hemi"])
    right=pd.read_table(s_vep, sep="\t", names=["id", "sym", "feat", "cons", "gnomAF", "CDSps", "exon", "intr", "prot_ps", "AmAc", "sift", "polyphen", "cadd", "carr"])
    ...
    if dup["cons"].isin(effect_var).any():
        with open("Variant_search_" + pop_input + ".txt", "a+") as final:
           ...
           #data_ok=out.to_string(na_rep="NA", index=False)
           sym=list(regions.gene)
           if regions.crom.eq(regions.crom.shift()):

           df_fin=out[out["ps"].isin(range(regions.start, regions.end))]
           subset=df_fin.POS in range(regions.start, regions.end)
           #gene=[ regions.loc for gen in regions.gene]
           final.write(data_ok)
           final.close()
    else:
        ...
        if dec == "y":
            with open("Variant_search_" + pop_input + ".txt", "a+") as final:
                  pd.set_option('max_colwidth', -1)
                  head=["CHROM", "POS", "ID", "REF", "ALT", "AC", "AN", "AF", "MAF", "AC_Het", "AC_Hom", "AC_Hemi", "Gene", "Isoform", "Consequence", "gnomAD_AF", "CDS_position", "Exon", "Intron", "Protein_position", "Amino_acids", "SIFT", "PolyPhen", "CADD_RAW", "Carriers"]
                  data_ok=dup.to_string(header=head, na_rep="NA", index=False)
                  final.write(data_ok)
                  final.close()
        else:
            os.remove(fill)
