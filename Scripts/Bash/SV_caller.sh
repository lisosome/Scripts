...
do
  ...
  mosdepth -n --quantize 0:1:4:150: ${out_file} ${cram}
...
#do
  ...
  #paste -d " " <(echo "${sam}") <(echo "${old_path}") <(echo "${new_path}") >> ${Mout}/SV_updated_manifest.tsv
#done < ${Mout}/SV_manifest.tsv
conda deactivate
...
python 2.7 ${PathMan}/configManta.py --bam ${bam} --referenceFasta ${ref_genome} --runDir ${Mout}/1.Manta_outs/${sam}/Manta_run
#let's call the SVs!
res=${Mout}/1.Manta_outs/${sam}/Manta_run/results/diploidSV.vcf.gz
samtools=/large/___HOME___/burlo/nardone/.conda/envs/Manta/share/manta-1.6.0-1/libexec/samtools
python 2.7 ${Mout}/1.Manta_outs/${sam}/Manta_run/runWorkflow.py
python 2.7 /large/___HOME___/burlo/nardone/.conda/envs/Manta/sha-1/libexec/convertInversion.py ${samtools} ${ref_genome} ${res} > ${Mout}/1.Manta_outs/${sam}/${sam}_final.vcf
done < ${Mout}/SV_updated_manifest.tsv
conda deactivate
...
do
  sam=$(echo $line | awk '{print $1}')
  bcftools view ${delout}/${sam}_geno.bcf > ${vcfs}/${sam}_geno.vcf && bcftools view -i "FILTER=='PASS'" -Ov -o ${filter}/${sam}_filtered.vcf
done < ${Mout}/SV_updated_manifest.tsv
...
lenght=$(wc -l ${Mout}/SV_updated_manifest.tsv )
readarray -t bams < <(awk '{print $2}' ${Mout}/SV_updated_manifest.tsv)
bam_p=$(echo "${bams[@]}")
...
do
  sam=$(echo $line | awk '{print $1}')
  find -L ${Mout}/1.Manta_outs/${sam} -type f -name "*.vcf" >> ${merlis}/${sam}_files_to_merge.txt
  find -L ${filter} -type f -name "*.vcf" >> ${merlis}/${sam}_files_to_merge.txt
  find -L ${smVCF} -type f -name "*.vcf" >> ${merlis}/${sam}_files_to_merge.txt
done < ${Mout}/SV_updated_manifest.tsv
...
do
  sam=$(echo $line | awk '{print $1}')
  ${svpack} consequence ${sur}/${sam}_unnotated.vcf ${gff} > ${ann}/${sam}_annotated.vcf
  ${svpack} consequence --require-csq ${sur}/${sam}_unnotated.vcf ${gff} > ${cod}/${sam}_ann_coding_only.vcf
done < ${Mout}/SV_updated_manifest.tsv
conda deactivate
